...
public class LaunchSmokeTest {
    ...
    public void launcherOpens() {
        onView(withText("Pressure MAG Recorder v7.3")).check(matches(isDisplayed()));
        onView(withText("Open Health Recorder v7.3")).check(matches(isDisplayed()));
    }

    @Test
    public void recorderScreenOpensFromLauncher() {
        onView(withText("Open Health Recorder v7.3")).perform(click());
        onView(withText("Pressure + MAG Recorder v7.3")).check(matches(isDisplayed()));
